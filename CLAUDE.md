...
## 문항 구조 (`google_apps_script.js` `QUESTIONS`)
> **2026-09-16 변경** — 문항 100개 전문은 `test.js`에서 `google_apps_script.js`로 옮겼다.
> `test.js`는 공개 저장소로 배포되므로 거기 두면 지원자가 시험 전에 문항을 전부 읽을 수 있었다.
> 이제 `test.js`의 `allQuestions`는 빈 배열로 시작하고, 검사 시작 시 `loadQuestions()`가
> `doGet?action=questions`로 받아 채운다. **문항 수정은 `QUESTIONS` 배열에서 하고 재배포할 것.**
> 서버는 내려보낼 때 `category`를 뺀다(어느 역량을 재는 문항인지 알면 답을 꾸미기 쉬워지므로).
...
## 저장소 구성 (2026-09-15 개편)
| 저장소 | 공개 여부 | 용도 |
|---|---|---|
| `gnidlsworoqkf/GN-FIT` | **public** | 실제 서비스. GitHub Pages가 여기 main 브랜치를 배포한다 |
| `gnidlsworoqkf/GN-FIT-archive` | private | 개편 이전의 전체 개발 이력 보관용. 서비스와 무관 |

사이트 주소: `https://gnidlsworoqkf.github.io/GN-FIT/`

저장소를 나눈 이유: 이전 이력의 옛 `admin.html`·`report_v4.html` 커밋에 PART2 정답키가
남아 있어, 이력을 이어받지 않고 현재 상태에서 새 public 저장소로 새출발했다.
**옛 이력을 public 저장소로 다시 가져오지 말 것.**

public 저장소에 올라간 `.html`/`.js`는 전부 공개다 — 비밀로 해야 할 값은 절대 넣지 말 것.

## 관리자 인증 (ADMIN_TOKEN)
전체 응시자 조회는 토큰이 있어야만 된다. 토큰 값은 **Apps Script 편집기 →
[프로젝트 설정] → [스크립트 속성]**의 `ADMIN_TOKEN`에만 존재하며, 어떤 파일에도 적지 않는다.

- `index.html`/`admin.html`은 토큰의 정오를 **판정하지 않는다.** 입력값을 `sessionStorage`
  (`gnfitAdminToken`)에 담아 요청에 실어 보내고, 판정은 서버가 한다.
  (예전엔 `pw === '1234'`로 클라이언트에서 비교했는데, 공개 소스라 잠금 역할을 못 했다.)
- 서버는 `ADMIN_TOKEN`이 설정되지 않았으면 데이터를 **주지 않고 막는다**(fail closed).
  관리자 화면에 데이터가 안 나오면 먼저 이 속성이 설정돼 있는지 확인할 것.
  `{"error":"not_configured"}` = 속성 미설정 / `{"error":"unauthorized"}` = 토큰 불일치.
...
| `test.html` / `test.js` / `test.css` | 본 검사 진행 화면 (문항은 서버에서 받아옴) |
| `result.html` | 완료 화면 |
| `admin.html` | 관리자 대시보드 |
| `report_v4.html` | 개인 리포트 (PDF) |
| `google_apps_script.js` | 백엔드 (**문항 100개**·정답키·서버채점·시트 저장/조회) — **로컬 전용, git 미포함** |
| `serve.py` | 로컬 미리보기 서버 (배포와 무관) |
| `GNFit_논리구조_최신판.html` | 전체 로직 설계 문서 — **로컬 전용, git 미포함** |
| `test0202.js` / `test_backup_60q.js` | 구버전 백업 (사용 안 함) — **로컬 전용, git 미포함** |

## 절대 git에 올리지 않는 파일 (`.gitignore`로 차단됨)
`google_apps_script.js`, `GNFit_배점표_대외비.html`, `GN_역량검사_최신문항_전체정리.xlsx`, `첨_부_사단법인_역량가이드북.pdf`,
`GNFit_논리구조_최신판.html`, `test0202.js`, `test_backup_60q.js`
— 문항 전문·배점 기준·정답 근거가 들어있어 유출 시 검사가 무력화된다.
