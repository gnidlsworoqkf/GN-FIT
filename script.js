...
window.onload = function () {
    ...
    if (loginForm) {
        loginForm.onsubmit = async function (e) {
            ...
            try {
                // 중복 응시 확인은 서버에서 한다. 서버는 true/false만 돌려주고 개인정보는 반환하지 않는다.
                //
                // (과거에는 여기서 시트 전체를 받아와 브라우저가 중복을 판별했다. 그 방식은
                //  로그인 화면을 연 지원자 누구나 다른 지원자 전원의 성명·휴대폰번호·생년월일을
                //  내려받을 수 있다는 뜻이어서 제거했다.)
                const checkURL = `${scriptURL}?action=checkDuplicate&phone=${encodeURIComponent(phone)}&_cb=${Date.now()}`;
